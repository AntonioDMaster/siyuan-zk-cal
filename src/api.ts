/**
 * Copyright (c) 2026 AntonioDMaster. All rights reserved.
 * https://github.com/AntonioDMaster/siyuan-zk-cal
 */
...
export async function getTreeStat(id: BlockId): Promise<IResTreeStat | null> {
    const data = { id };
    const url = "/api/block/getTreeStat";
    const res = await request(url, data) as { stat?: IResTreeStat } | null;
    return res?.stat ?? null;
}
...
export async function getSystemConf(): Promise<IResGetSystemConf> {
